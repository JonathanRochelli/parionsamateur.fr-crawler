...
function main($j){
	$j.getScript( window.cmCDN + '/js/jquery.cookie.js', function(){
		if( typeof($j.cookie('cnil')) === "undefined" ){
			...
			$j(document).on('click', '.cookie-bar i, .cookie-bar a, body a', function(e){
                if(document.createStyleSheet){
                    $j('.cookie-bar').hide();
                } else {
			...
			var wcmTC = (typeof(window.cmTextContent) !== "undefined" && window.cmTextContent !== null)? true : false ;
			...
			var cmLink = " <a href='{href}' target='_blank'>{text}</a>";
